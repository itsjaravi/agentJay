...
def api_contract_creator_ui():
    # Set Streamlit page config
    #st.set_page_config(page_title="API Contract Generator", layout="wide")
    st.title("📄 AI-Powered API Contract Generator")
    st.markdown(
        "Fill in the API metadata below and click **Generate Contract** to create a complete API documentation."
    )
    ...
    with st.form("contract_form"):
        ...
        auth_method = st.selectbox(
            "Authentication Method", ["JWT Bearer", "API Key", "Basic Auth"]
        )
        ...
        auth_header_example = st.text_input(
            "Example Authorization Header", value="Authorization: Bearer <token>"
        )
        global_headers = st.text_area(
            "Global Headers",
            value="- Authorization: string (required) - JWT access token\n- Content-Type: string (required) - MIME type",
        )
        entities = st.text_area(
            "Entities",
            value="- Task: title, description, dueDate, priority, status\n- Project: name, description, owner\n- User: name, email, role",
        )
    ...
    if submit:
        with st.spinner("Generating API Contract..."):
            ...
            try:
                ...
                # Optional: Enable these if you want download options
